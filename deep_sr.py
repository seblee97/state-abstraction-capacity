...
class DSRAgent:
    ...
    def update(self) -> Optional[dict]:
        ...
        # Compute SR target (all target computations should be detached)
        # Target: phi(s) + gamma * psi(s', a')  where a' = argmax_a Q(s', a)
        # Note: phi(s) in target should be detached - it's the "immediate feature reward"
        with torch.no_grad():
            # Use target network for next state
            psi_next = self.target_network.successor_features(next_states)
            q_next = self.target_network.q_values(next_states)
            next_actions = q_next.argmax(dim=1)
            psi_next_sa = psi_next[batch_indices, next_actions]

            # phi(s) should also be detached in the target
            # SR Bellman: psi(s,a) = phi(s) + gamma * psi(s', a')
            # At terminal states, there's no future, so: psi(s,a) = phi(s)
            # phi_detached = phi.detach()
            # sr_target = phi_detached + cfg.gamma * psi_next_sa * (1 - dones.unsqueeze(1))

            # NEW: terminal next-state feature
            phi_next, _, _, _ = self.target_network(next_states)
            phi_next = phi_next.detach()

            done = dones.unsqueeze(1)

            # nonterminal -> psi_next_sa, terminal -> phi_next
            bootstrap = (1 - done) * psi_next_sa + done * phi_next

            phi_detached = phi.detach()
            sr_target = phi_detached + cfg.gamma * bootstrap

        # SR Loss: MSE between predicted psi(s,a) and target
        sr_loss = F.mse_loss(psi_sa, sr_target)

        # Reward prediction loss: w @ phi(s) should predict r
        # This DOES backprop through phi (we want phi to be predictive of reward)
        predicted_reward = self.network.reward_weights(phi).squeeze(-1)
